...
version: '2.0'
api:
  url: /space/adGroup/listWithAssignment
  method: POST
  name: 获取桌面池 AD 安全组分配信息列表（已分配优先，分页合并未分配）
  controller: SpaceAdUserController
  method_ref: getUserWithAssignmentList
  permission: 无
  exec_mode: sync
  async: false
  description: 获取桌面池 AD 安全组分配信息列表（已分配优先，分页合并未分配）
setup:
- name: login
...
---
# POST /space/adGroup/listWithAssignment

> 获取桌面池 AD 安全组分配信息列表（已分配优先，分页合并未分配） ｜ 无特殊权限 ｜ sync
...
graph LR
    ...
    B["POST /space/adGroup/listWithAssignment<br>获取桌面池 AD 安全组分配信息列表（已分配优先，分页合并未分配）<br>权限: 无"]
    A1 -->|数据| B
    subgraph 内部处理流程
        C1["Step1: Assert request 非空"]
        C2["Step2: 解析 classroomId（matchArr）"]
        C3["Step3: 分页合并已分配与未分配 AD 安全组（pageQueryPoolAdGroup）"]
...
| 权限注解 | 无 |
| 执行方式 | sync |
| 业务含义 | 获取桌面池 AD 安全组分配信息列表（已分配优先，分页合并未分配） |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| itemArr | AdGroupListDTO[] | AD安全组列表 |
| total | Long | 总数 |
...
1. Assert request 非空
2. 解析 classroomId（matchArr）
3. 分页合并已分配与未分配 AD 安全组（pageQueryPoolAdGroup）
4. 返回 AdGroupListDTO 分页

## 下游消费方

### 消费1：POST /space/adGroup/listWithAssignment

AD安全组ID，被 /rcc/space/user/update selectedAdGroupIdList 消费（推断字段名 adGroupId/id）（由 field_map 契约映射）
## 接口参数约束分析
