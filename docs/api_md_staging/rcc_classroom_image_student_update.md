...
version: '2.0'
api:
  url: /rcc/classroom/image/student/update
  method: POST
  name: 更新学生机课程镜像（action=UPDATE），同步执行镜像更新并通知学生端刷新镜像列表
  controller: RccClassroomImageController
  method_ref: studentDoUpdate
  permission: '@EnableAuthority'
  exec_mode: sync
  async: false
  description: 更新学生机课程镜像（action=UPDATE），同步执行镜像更新并通知学生端刷新镜像列表
setup:
- name: login
...
request:
  ...
  body:
    ...
    shouldOnlyDeleteDataFromDb:
      ...
      required: false
      constraint: 可空
      description: 仅删除场景使用，UPDATE 下忽略
response:
  wrapper:
    status: String
    message: String
...
---
# POST /rcc/classroom/image/student/update

> 更新学生机课程镜像（action=UPDATE），同步执行镜像更新并通知学生端刷新镜像列表 ｜ @EnableAuthority ｜ sync
...
graph LR
    ...
    B["POST /rcc/classroom/image/student/update<br>更新学生机课程镜像（action=UPDATE），同步执行镜像更新并通知学生端刷<br>权限: @EnableAuthority"]
    A1 -->|数据| B
    A2 -->|数据| B
    subgraph 内部处理流程
...
| 执行方式 | sync |
| 业务含义 | 更新学生机课程镜像（action=UPDATE），同步执行镜像更新并通知学生端刷新镜像列表 |
...
| shouldOnlyDeleteDataFromDb | Boolean | 否 | 可空 | 仅删除场景使用，UPDATE 下忽略 |
