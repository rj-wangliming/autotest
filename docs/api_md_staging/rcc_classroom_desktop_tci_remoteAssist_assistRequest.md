...
version: '2.0'
api:
  url: /rcc/classroom/desktop/tci/remoteAssist/assistRequest
  method: POST
  name: 发起TCI桌面远程协助：权限校验后构造 RemoteAssistRequest 提交协助，成功后清理协助缓存。
  controller: TCIDesktopOperateController
  method_ref: tciRemoteAssistance
  permission: '@EnableAuthority'
  exec_mode: sync
  async: false
  description: 发起TCI桌面远程协助：权限校验后构造 RemoteAssistRequest 提交协助，成功后清理协助缓存。
setup:
- name: login
...
- level: BIZ
  field: desktop
  rule: 桌面必须存在
  failure: 返回 rcdc_rcc_tci_desktop_remote_assist_fail
assertions:
  success:
  - scenario: 桌面存在且有权限
  ...
  - scenario: 桌面不存在或协助申请失败
    trigger: applyRemoteAssist 抛 BusinessException
    expect: status==ERROR；msgKey==rcdc_rcc_tci_desktop_remote_assist_fail
cleanup:
- api: 无对应 HTTP 清理接口
...
> 发起TCI桌面远程协助：权限校验后构造 RemoteAssistRequest 提交协助，成功后清理协助缓存。 ｜ @EnableAuthority ｜ sync
...
graph LR
    ...
    B["POST /rcc/classroom/desktop/tci/remoteAssist/assistRequest<br>发起TCI桌面远程协助：权限校验后构造 RemoteAssistRequest <br>权限: @EnableAuthority"]
    A1 -->|数据| B
    subgraph 内部处理流程
        C1["Step1: 断言 request 与 session 非空"]
        C2["Step2: 权限校验（id）"]
        C3["Step3: obtainDesktopName(id) 取桌面名"]
        C4["Step4: 构造 RemoteAssistRequest(id, userId, userN"]
        C5["Step5: remoteAssistInquire.applyRemoteAssist 发起"]
        C6["Step6: deleteRemoteAssistanceCache 清理缓存"]
        C1 --> C2
        C7["Step7: 记成功审计返回 success；catch 记失败审计返回 fail"]
        C6 --> C7
        C2 --> C3
        C3 --> C4
        C4 --> C5
...
| URL | /rcc/classroom/desktop/tci/remoteAssist/assistRequest |
| Controller | TCIDesktopOperateController |
| 方法名 | tciRemoteAssistance |
| 权限注解 | @EnableAuthority |
| 执行方式 | sync |
| 业务含义 | 发起TCI桌面远程协助：权限校验后构造 RemoteAssistRequest 提交协助，成功后清理协助缓存。 |
...
4. 构造 RemoteAssistRequest(id, userId, userName)，desktopMgmtAPI.getDeskIDV 设置 resourceId/deskType
5. remoteAssistInquire.applyRemoteAssist 发起协助
6. deleteRemoteAssistanceCache 清理缓存
7. 记成功审计返回 success；catch 记失败审计返回 fail
...
| BIZ | desktop | 桌面必须存在 | 返回 rcdc_rcc_tci_desktop_remote_assist_fail |
...
| 场景 | 断言点 |
|---|---|
| 桌面存在且有权限 | $.status=="SUCCESS"（content 为空，Builder.success() 无参） |

### 失败场景

| 场景 | 触发条件 | 断言点 |
|---|---|---|
| 桌面不存在或协助申请失败 | applyRemoteAssist 抛 BusinessException | status==ERROR；msgKey==rcdc_rcc_tci_desktop_remote_assist_fail |
