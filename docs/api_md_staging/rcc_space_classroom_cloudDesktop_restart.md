...
version: '2.0'
api:
  url: /rcc/space/classroom/cloudDesktop/restart
  method: POST
  name: 教学桌面池教室下所有云桌面重启。逻辑与强制关机一致：idArr[0] 教室ID → 查询教室云桌面ID（含权限过滤）→ 空则成功返回 → 构造 RestartD
  controller: RccSpaceController
  method_ref: restart
  permission: '@EnableAuthority'
  exec_mode: 批量异步（BatchTask）
  async: true
  description: 教学桌面池教室下所有云桌面重启。逻辑与强制关机一致：idArr[0] 教室ID → 查询教室云桌面ID（含权限过滤）→ 空则成功返回 → 构造 RestartDesktopBatchTaskHandler 批量任务（单台 SINGLE_TASK / 多台 enableParallel，itemName=RCDC_RCC_DESKTOP_RESTART_ITEM_NAME）。
setup:
- name: login
...
downstream:
- api: 内部调用:rcc/RccDesktopOperateAPI#restart
...
assertions:
  success:
  - scenario: 教室存在云桌面
    expect: 提交重启批量任务，返回 BatchTaskSubmitResult；轮询 content.taskId（2000ms 间隔）至终态 batchTaskItemStatus∈["SUCCESS"]
  failure:
  - scenario: 无桌面
    trigger: 教室下无云桌面
...
---
# POST /rcc/space/classroom/cloudDesktop/restart

> 教学桌面池教室下所有云桌面重启。逻辑与强制关机一致：idArr[0] 教室ID → 查询教室云桌面ID（含权限过滤）→ 空则成功返回 → 构造 RestartDesktopBatchTaskHandler 批量任务（单台 SINGLE_TASK / 多台 enableParallel，itemName=RCDC_RCC_DESKTOP_RESTART_ITEM_NAME）。 ｜ @EnableAuthority ｜ 批量异步（BatchTask）
...
graph LR
    ...
    B["POST /rcc/space/classroom/cloudDesktop/restart<br>教学桌面池教室下所有云桌面重启。逻辑与强制关机一致：idArr[0] 教室ID <br>权限: @EnableAuthority"]
    A1 -->|数据| B
    subgraph 内部处理流程
        ...
        C5["Step5: 构造 RestartDesktopBatchTaskHandler 注入 des"]
        C6["Step6: executeRestartBatchTask：单台 SINGLE_TASK 命"]
        C1 --> C2
...
| 执行方式 | 批量异步（BatchTask） |
| 业务含义 | 教学桌面池教室下所有云桌面重启。逻辑与强制关机一致：idArr[0] 教室ID → 查询教室云桌面ID（含权限过滤）→ 空则成功返回 → 构造 RestartDesktopBatchTaskHandler 批量任务（单台 SINGLE_TASK / 多台 enableParallel，itemName=RCDC_RCC_DESKTOP_RESTART_ITEM_NAME）。 |
...
| 2 | desktopOperateAPI.restart(desktopId) 下发重启命令 |
| 3 | 成功：auditLogAPI.recordLog(RCDC_RCC_DESKTOP_RESTART_SUC_LOG) |
| 4 | 失败：recordLog(RCDC_RCC_DESKTOP_RESTART_FAIL_LOG)，返回 FAILURE |
| 5 | onFinish：单台 RCDC_RCC_DESKTOP_RESTART_SINGLE_SUC/FAIL，多台 RCDC_RCC_DESKTOP_RESTART_BATCH_RESULT |
...
4. 构造 DefaultBatchTaskItem（itemName=RCDC_RCC_DESKTOP_RESTART_ITEM_NAME）
5. 构造 RestartDesktopBatchTaskHandler 注入 desktopOperateAPI/desktopMgmtAPI
6. executeRestartBatchTask：单台 SINGLE_TASK 命名，多台 enableParallel 批量任务
...
| 场景 | 断言点 |
|---|---|
| 教室存在云桌面 | 提交重启批量任务，返回 BatchTaskSubmitResult |
...
| 重启命令下发失败 | 桌面不存在或状态异常 | 轮询 content.taskId 至终态 batchTaskItemStatus∈["FAILURE"] |
## 环境清理机制
