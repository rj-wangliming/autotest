...
version: '2.0'
api:
  url: /rcc/classroom/image/teacher/hide
  method: POST
  name: 隐藏教师机课程镜像（action=HIDE），同步执行镜像隐藏并通知教师机刷新镜像
  controller: RccClassroomImageController
  method_ref: teacherDoHide
  permission: '@EnableAuthority'
  exec_mode: sync
  async: false
  description: 隐藏教师机课程镜像（action=HIDE），同步执行镜像隐藏并通知教师机刷新镜像
setup:
- name: login
...
---
# POST /rcc/classroom/image/teacher/hide

> 隐藏教师机课程镜像（action=HIDE），同步执行镜像隐藏并通知教师机刷新镜像 ｜ @EnableAuthority ｜ sync
...
graph LR
    ...
    B["POST /rcc/classroom/image/teacher/hide<br>隐藏教师机课程镜像（action=HIDE），同步执行镜像隐藏并通知教师机刷新镜<br>权限: @EnableAuthority"]
    A1 -->|数据| B
    A2 -->|数据| B
    subgraph 内部处理流程
...
| 执行方式 | sync |
| 业务含义 | 隐藏教师机课程镜像（action=HIDE），同步执行镜像隐藏并通知教师机刷新镜像 |
...
4. doImageAction()：构建 DoCardActionRequest → classroomImageAPI.doAction(request) 执行隐藏
