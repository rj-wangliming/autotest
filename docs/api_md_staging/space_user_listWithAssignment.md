...
version: '2.0'
api:
  url: /space/user/listWithAssignment
  method: POST
  name: 获取用户及分配信息列表：已分配/未分配分页合并，已分配优先展示，附带是否绑定桌面池标记
  controller: SpaceUserController
  method_ref: getUserWithAssignmentList
  permission: 无
  exec_mode: sync
  async: false
  description: 获取用户及分配信息列表：已分配/未分配分页合并，已分配优先展示，附带是否绑定桌面池标记
setup:
- name: login
...
request:
  ...
  body:
    ...
    matchArr:
      type: Match[]
      required: true
      constraint: 需含 classroomId 等匹配条件
      description: 查询条件
    sortArr:
      type: Sort[]
      required: true
...
---
# POST /space/user/listWithAssignment

> 获取用户及分配信息列表：已分配/未分配分页合并，已分配优先展示，附带是否绑定桌面池标记 ｜ 无特殊权限 ｜ sync
...
graph LR
    ...
    B["POST /space/user/listWithAssignment<br>获取用户及分配信息列表：已分配/未分配分页合并，已分配优先展示，附带是否绑定桌面<br>权限: 无"]
    A1 -->|数据| B
    subgraph 内部处理流程
        C1["Step1: Assert request/sessionContext 非空"]
        C2["Step2: buildAssignmentQueryContext 解析 classroom"]
        C3["Step3: 计算 total = countBaseUserTotal(matchList)"]
        C4["Step4: pageQueryPoolUser 查已分配用户当前页；findBindDesk"]
        C5["Step5: 当前页完全落在已分配区 → 直接返回；否则按偏移查未分配区并拼接/偏移处理"]
        C6["Step6: convert2UserListDTOArr 设置 hasBindPoolDes"]
        C1 --> C2
        C2 --> C3
        C3 --> C4
        C4 --> C5
...
| URL | /space/user/listWithAssignment |
| Controller | SpaceUserController |
| 方法名 | getUserWithAssignmentList |
| 权限注解 | 无 |
| 执行方式 | sync |
| 业务含义 | 获取用户及分配信息列表：已分配/未分配分页合并，已分配优先展示，附带是否绑定桌面池标记 |
...
| matchArr | Match[] | 是 | 需含 classroomId 等匹配条件 | 查询条件 |
| sortArr | Sort[] | 是 | 排序条件 | 排序 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| itemArr | UserListDTO[] | 用户列表（已分配在前） |
| total | Long | 用户总数 |
...
3. 计算 total = countBaseUserTotal(matchList)
4. pageQueryPoolUser 查已分配用户当前页；findBindDeskUserIdSet 预取已绑定桌面用户ID
5. 当前页完全落在已分配区 → 直接返回；否则按偏移查未分配区并拼接/偏移处理
6. convert2UserListDTOArr 设置 hasBindPoolDesktop 标记后返回

## 下游消费方

### 消费1：POST /space/user/listWithAssignment

用户ID，被 /rcc/space/user/update addUserByIdList 消费（由 field_map 契约映射）
## 接口参数约束分析
...
| 页码超出总数 | $.content.itemArr 为空 |
| 正常分页 | $.content.itemArr 非空 |
