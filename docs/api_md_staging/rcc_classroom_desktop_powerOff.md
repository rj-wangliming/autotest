...
version: '2.0'
api:
  url: /rcc/classroom/desktop/powerOff
  method: POST
  name: 课堂云桌面强制关机：对指定VDI桌面批量下发强制关机指令（shutdownByAdmin=true）。
  controller: RccClassroomDesktopController
  method_ref: powerOff
  permission: '@EnableAuthority'
  exec_mode: batch
  async: false
  description: 课堂云桌面强制关机：对指定VDI桌面批量下发强制关机指令（shutdownByAdmin=true）。
setup:
- name: login
...
assertions:
  ...
  - scenario: 桌面存在且允许强制关机
    expect: 批量任务提交成功，逐台强制关机成功；轮询 content.taskId（2000ms 间隔）至终态 batchTaskItemStatus∈["SUCCESS"]
  failure:
...
> 课堂云桌面强制关机：对指定VDI桌面批量下发强制关机指令（shutdownByAdmin=true）。 ｜ @EnableAuthority ｜ batch
...
graph LR
    ...
    B["POST /rcc/classroom/desktop/powerOff<br>课堂云桌面强制关机：对指定VDI桌面批量下发强制关机指令（shutdownByA<br>权限: @EnableAuthority"]
    A1 -->|数据| B
    subgraph 内部处理流程
        C1["Step1: 断言 request 与 builder 非空"]
        C2["Step2: 取 idArr 并 distinct 构建任务项迭代器"]
        C3["Step3: 创建 RccPowerOffDesktopBatchTaskHandler 并注"]
        C4["Step4: 单条查询桌面名设置单任务，多条 enableParallel"]
        C5["Step5: 提交批量任务返回结果"]
        C1 --> C2
        C2 --> C3
        C3 --> C4
...
| 执行方式 | batch |
| 业务含义 | 课堂云桌面强制关机：对指定VDI桌面批量下发强制关机指令（shutdownByAdmin=true）。 |
...
| 3 | desktopOperateAPI.powerOff(shutdownDesktopDTO) 执行强制关机 |
| 4 | 成功记 RCDC_RCC_DESKTOP_POWEROFF_SUC_LOG，失败记 FAIL_LOG 并返回 FAILURE 项 |

### 处理流程

1. 断言 request 与 builder 非空
2. 取 idArr 并 distinct 构建任务项迭代器
3. 创建 RccPowerOffDesktopBatchTaskHandler 并注入 desktopOperateAPI/desktopMgmtAPI
4. 单条查询桌面名设置单任务，多条 enableParallel
5. 提交批量任务返回结果
...
| BIZ | desktop | 桌面必须存在 | 单项失败 rcdc_rcc_desktop_poweroff_item_fail_desc |
...
| 场景 | 断言点 |
|---|---|
| 桌面存在且允许强制关机 | 批量任务提交成功，逐台强制关机成功；轮询 content.taskId 至终态 batchTaskItemStatus∈["SUCCESS"] |

### 失败场景

| 场景 | 触发条件 | 断言点 |
|---|---|---|
| 桌面不存在或关机命令异常 | 桌面已删除或平台返回异常 | $.status=="SUCCESS"；content.taskId 非空；轮询终态对应项 batchTaskItemStatus==FAILURE；对应项 msgKey==rcdc_rcc_desktop_poweroff_item_fail_desc（单条任务时 finish msgKey==rcdc_rcc_desktop_poweroff_single_fail）；rcdc_rcc_desktop_poweroff_fail_log 仅为审计日志 key，不是响应 msgKey |
...
|---|---|
| 幂等性 | low |
| 说明 | 强制关机为有状态操作，重复执行对已关机桌面会重复下发断电指令；任务级不幂等 |
