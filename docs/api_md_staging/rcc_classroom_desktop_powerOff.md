...
assertions:
  ...
  - scenario: 桌面不存在或关机命令异常
    ...
    expect: $.status=="SUCCESS"；content.taskId 非空；轮询终态对应项 batchTaskItemStatus==FAILURE；msgKey==rcdc_rcc_desktop_poweroff_fail_log
cleanup: []
idempotency:
  level: data_level
...
| 场景 | 触发条件 | 断言点 |
|---|---|---|
| 桌面不存在或关机命令异常 | 桌面已删除或平台返回异常 | $.status=="SUCCESS"；content.taskId 非空；轮询终态对应项 batchTaskItemStatus==FAILURE；msgKey==rcdc_rcc_desktop_poweroff_fail_log |
