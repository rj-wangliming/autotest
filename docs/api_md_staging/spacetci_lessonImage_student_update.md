...
version: '2.0'
api:
  url: /spacetci/lessonImage/student/update
  method: POST
  name: 更新学生机课程镜像（如镜像模板版本变化）并异步推送镜像列表到座位
  controller: TCILessonImageController
  method_ref: updateStudentLessonImage
  permission: '@EnableAuthority'
  exec_mode: sync
  async: false
  description: 更新学生机课程镜像（如镜像模板版本变化）并异步推送镜像列表到座位
setup:
- name: login
...
response:
  ...
  body:
    msgKey:
      type: String
      description: spacetci_lessonimage_update_student_image_success_log
upstream:
- api: POST /spacetci/lessonImage/getLessonImageList
...
assertions:
  success:
  - scenario: 更新学生机镜像
    expect: $.status==SUCCESS（content 为空，msgKey==spacetci_lessonimage_update_student_image_success_log）
  failure:
  - scenario: 操作教师机镜像
    trigger: assertTeacherImageFalse抛异常
    expect: $.status==ERROR && $.msgKey==spacetci_lessonimage_update_student_image_fail_log
cleanup: []
idempotency:
  level: data_level
  note: 重复更新同一镜像状态一致，但每次触发推送批处理
params:
  required:
...
> 更新学生机课程镜像（如镜像模板版本变化）并异步推送镜像列表到座位 ｜ @EnableAuthority ｜ sync
...
graph LR
    ...
    B["POST /spacetci/lessonImage/student/update<br>更新学生机课程镜像（如镜像模板版本变化）并异步推送镜像列表到座位<br>权限: @EnableAuthority"]
    A1 -->|数据| B
    subgraph 内部处理流程
...
| URL | /spacetci/lessonImage/student/update |
| Controller | TCILessonImageController |
| 方法名 | updateStudentLessonImage |
| 权限注解 | @EnableAuthority |
| 执行方式 | sync |
| 业务含义 | 更新学生机课程镜像（如镜像模板版本变化）并异步推送镜像列表到座位 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| msgKey | String | spacetci_lessonimage_update_student_image_success_log |
...
| 场景 | 断言点 |
|---|---|
| 更新学生机镜像 | $.status==SUCCESS（content 为空，msgKey==spacetci_lessonimage_update_student_image_success_log） |

### 失败场景

| 场景 | 触发条件 | 断言点 |
|---|---|---|
| 操作教师机镜像 | assertTeacherImageFalse抛异常 | $.status==ERROR && $.msgKey==spacetci_lessonimage_update_student_image_fail_log |
