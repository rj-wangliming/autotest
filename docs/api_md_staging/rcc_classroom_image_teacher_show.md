...
version: '2.0'
api:
  url: /rcc/classroom/image/teacher/show
  method: POST
  name: 显示教师机课程镜像（action=SHOW），同步执行镜像显示并通知教师机刷新镜像
  controller: RccClassroomImageController
  method_ref: teacherDoShow
  permission: '@EnableAuthority'
  exec_mode: sync
  async: false
  description: 显示教师机课程镜像（action=SHOW），同步执行镜像显示并通知教师机刷新镜像
setup:
- name: login
...
> 显示教师机课程镜像（action=SHOW），同步执行镜像显示并通知教师机刷新镜像 ｜ @EnableAuthority ｜ sync
...
graph LR
    ...
    B["POST /rcc/classroom/image/teacher/show<br>显示教师机课程镜像（action=SHOW），同步执行镜像显示并通知教师机刷新镜<br>权限: @EnableAuthority"]
    A1 -->|数据| B
    A2 -->|数据| B
    subgraph 内部处理流程
...
| 执行方式 | sync |
| 业务含义 | 显示教师机课程镜像（action=SHOW），同步执行镜像显示并通知教师机刷新镜像 |
