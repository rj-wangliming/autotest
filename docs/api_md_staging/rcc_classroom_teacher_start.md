---
version: '2.0'
api:
  url: /rcc/classroom/teacher/start
  method: POST
  name: 启动教师机桌面（按教室+镜像，区分VOI/VDI镜像类型执行）
  controller: RccClassroomLessonController
  method_ref: startTeacherDesktop
  permission: '@EnableAuthority'
  exec_mode: sync
  async: false
  description: 启动教师机桌面（按教室+镜像，区分VOI/VDI镜像类型执行）
setup:
- name: login
  api: POST /rco/admin/loginAdmin
  purpose: 管理员登录（框架内置，引擎自动处理）
- name: create_classroom
  api: POST /rcc/classroom/create
  purpose: 创建教室（异步批处理任务，出参BatchTaskSubmitResult）
  request:
    body:
      classroomName: ${param.classroom_name}
  idempotent: recreate
  delete_api: /rcc/classroom/delete
  delete_param: classroomId
- name: query_classroom
  api: POST /rcc/classroom/select
  extract:
    classroomId: $.content[0].classroomId
  purpose: 按教室名精确过滤（matchArr.fieldName=classroomName）
  request:
    body:
      matchArr:
      - type: EXACT
        fieldName: classroomName
        valueArr:
        - ${param.classroom_name}
        matchRule: EQ
- name: query_image
  api: POST /rcc/classroom/image/list
  extract:
    imageId: $.content.itemArr[0].id
  purpose: 按镜像名精确过滤（searchKeyword + matchArr.fieldName=imageName）
  request:
    body:
      searchKeyword: ${param.student_image_name}
      matchArr:
      - type: EXACT
        fieldName: imageName
        valueArr:
        - ${param.image_name}
        matchRule: EQ
request:
  dto: OperateTeacherDesktopWebRequest
  body:
    classroomId:
      type: UUID
      required: true
      constraint: '@NotNull 非空'
      description: 教室ID
      value: ${prev.query_classroom.output.classroomId}
    imageId:
      type: UUID
      required: true
      constraint: '@NotNull 非空'
      description: 教师机桌面镜像ID
      value: ${prev.query_image.output.imageId}
response:
  wrapper:
    status: String
    message: String
    msgKey: String
    msgArgArr: String[]
    content: Object
  body:
    content:
      type: Object
      description: 纯操作接口：content 为空（仅返回 msgKey，无 content body）
upstream:
- api: POST /rcc/classroom/terminal/list
  produces: $.content.itemArr[0].classroomId
  purpose: 教室ID在创建教室(POST /rcc/classroom/create)后经教室终端列表查询获得（ViewClassroomInfoEntity.classroomId）
- api: POST /rcc/classroom/image/list
  produces: $.content.itemArr[0].imageId
  purpose: 推断：镜像ID来源，字段名为推断
downstream:
- api: 内部调用:LessonFactory
  purpose: 内部调用（非 HTTP 端点）
constraints:
- level: request
  field: classroomId/imageId
  rule: '@NotNull 非空'
  failure: webmvc 参数校验异常
- level: business
  field: terminalGroupId
  rule: 管理员需具备教室终端组数据权限
  failure: space_lesson_permission_denied
assertions:
  success:
  - scenario: 镜像存在且启动成功
    expect: status==SUCCESS；content 为空；msgKey==SPACETCI_LESSON_TEACHER_START_LESSON_SUCCESS_LOG
  failure:
  - scenario: 权限不足
    trigger: 非授权管理员（checkPermission 失败）
    expect: status==ERROR；msgKey==SPACE_LESSON_PERMISSION_DENIED
  - scenario: 启动失败
    trigger: 镜像不存在/资源不足/lessonService.startTeacherDesktop 抛 BusinessException
    expect: status==ERROR；msgKey==SPACETCI_LESSON_TEACHER_START_LESSON_FAIL_LOG
  - scenario: 入参为空
    trigger: classroomId/imageId 未传
    expect: status==ERROR（参数校验 Assert.notNull）
cleanup: []
prereq_state:
  resource: teacher
  required_state: CLOSE
  achieve_via: []

idempotency:
  level: data_level
  note: 重复启动同一教师机桌面通常无副作用，但无显式防重，仍会重新下发启动指令
params:
  required:
  - name: classroom_name
    desc: ''
    used_by: 见 setup/request
  - name: student_image_name
    desc: ''
    used_by: 见 setup/request
  - name: image_name
    desc: ''
    used_by: setup/request
---
# POST /rcc/classroom/teacher/start

> 启动教师机桌面（按教室+镜像，区分VOI/VDI镜像类型执行） ｜ @EnableAuthority ｜ sync

## 依赖关系全景图

```mermaid
graph LR
    subgraph 上游前置业务
        A1["POST /rcc/classroom/terminal/list"]
        A2["POST /rcc/classroom/image/list"]
    end
    B["POST /rcc/classroom/teacher/start<br>启动教师机桌面（按教室+镜像，区分VOI/VDI镜像类型执行）<br>权限: @EnableAuthority"]
    A1 -->|数据| B
    A2 -->|数据| B
    subgraph 内部处理流程
        C1["Step1: Assert webRequest/sessionContext 非空"]
        C2["Step2: 查教室信息，权限校验（SPACE_LESSON_PERMISSION_DENIE"]
        C3["Step3: classroomImageAPI.getImageName 取镜像名"]
        C4["Step4: lessonService.startTeacherDesktop：按镜像类型 "]
        C5["Step5: 成功记录 SPACETCI_LESSON_TEACHER_START_LESSO"]
        C1 --> C2
        C2 --> C3
        C3 --> C4
        C4 --> C5
    end
    B --> C1
    subgraph 下游消费方
        D1["（无 HTTP 下游）"]
    end
    B -->|数据| D1
```

## 接口基本信息

| 项目 | 内容 |
|---|---|
| URL | /rcc/classroom/teacher/start |
| Controller | RccClassroomLessonController |
| 方法名 | startTeacherDesktop |
| 权限注解 | @EnableAuthority |
| 执行方式 | sync |
| 业务含义 | 启动教师机桌面（按教室+镜像，区分VOI/VDI镜像类型执行） |

## 入参详情

### OperateTeacherDesktopWebRequest

| 参数名 | 类型 | 必填 | 约束 | 说明 |
|---|---|---|---|---|
| classroomId | UUID | 是 | @NotNull 非空 | 教室ID |
| imageId | UUID | 是 | @NotNull 非空 | 教师机桌面镜像ID |

## 出参详情

| 返回类型 | DefaultWebResponse（成功，无 data） |
| 说明 | 纯操作接口：content 为空（仅返回 msgKey，无 content body）；成功返回 SUCCESS；失败返回 status/msgKey |
|---|---|

| 字段 | 类型 | 说明 |
|---|---|---|

## 上游前置业务

### 前置1：POST /rcc/classroom/terminal/list

教室ID在创建教室(POST /rcc/classroom/create)后经教室终端列表查询获得（ViewClassroomInfoEntity.classroomId）（由 field_map 契约映射）

### 前置2：POST /rcc/classroom/image/list

推断：镜像ID来源，字段名为推断（由 field_map 契约映射）
## 内部处理流程

### 处理流程

1. Assert webRequest/sessionContext 非空
2. 查教室信息，权限校验（SPACE_LESSON_PERMISSION_DENIED）
3. classroomImageAPI.getImageName 取镜像名
4. lessonService.startTeacherDesktop：按镜像类型 lessonFactory.create(...).startTeacherDesktop
5. 成功记录 SPACETCI_LESSON_TEACHER_START_LESSON_SUCCESS_LOG 并返回成功；异常记录失败审计并返回 fail

## 下游消费方

（本接口数据主要被自身/关联接口消费，或经 SPI 通知）
## 接口参数约束分析

| 层级 | 参数 | 规则 | 失败结果 |
|---|---|---|---|
| request | classroomId/imageId | @NotNull 非空 | webmvc 参数校验异常 |
| business | terminalGroupId | 管理员需具备教室终端组数据权限 | space_lesson_permission_denied |

## 参数取值策略

| 参数 | 策略 | 说明 |
|---|---|---|
| classroomId | user_input/from_query | 按业务构造 |
| imageId | user_input/from_query | 按业务构造 |

## 成功/失败断言基准

> ⚠️ 断言以 HTTP 响应为准（status + msgKey），非服务端审计日志。依据源码 RccClassroomLessonController.startTeacherDesktop（#234-265）。

### 成功场景

| 场景 | 触发条件 | 断言点 |
|---|---|---|
| 启动成功 | 权限通过 + 教室存在 + 镜像存在 + 启动无异常 | status==SUCCESS；content 为空；msgKey==SPACETCI_LESSON_TEACHER_START_LESSON_SUCCESS_LOG |

### 失败场景

| 场景 | 触发条件 | 断言点 |
|---|---|---|
| 权限不足 | 非授权管理员（checkPermission 失败 #243） | status==ERROR；msgKey==SPACE_LESSON_PERMISSION_DENIED |
| 启动失败 | 镜像不存在/资源不足/lessonService.startTeacherDesktop 抛 BusinessException | status==ERROR；msgKey==SPACETCI_LESSON_TEACHER_START_LESSON_FAIL_LOG |
| 入参为空 | classroomId/imageId 未传（Assert.notNull） | status==ERROR（参数校验 Assert.notNull） |

## 环境清理机制

| 接口 | 说明 |
|---|---|
| 本接口创建的资源 | 通过对应 delete 接口清理 |

## 前置状态和幂等性标注

| 维度 | 结论 |
|---|---|
| 幂等性 | medium |
| 说明 | 重复启动同一教师机桌面通常无副作用，但无显式防重，仍会重新下发启动指令 |
