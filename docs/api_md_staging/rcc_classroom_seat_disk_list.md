...
version: '2.0'
api:
  url: /rcc/classroom/seat/disk/list
  method: POST
  name: 查看座位终端的磁盘信息列表，按座位ID校验权限后分页返回
  controller: RccSeatManageController
  method_ref: getSeatDiskList
  permission: 无
  exec_mode: 同步（分页查询）
  async: false
  description: 查看座位终端的磁盘信息列表，按座位ID校验权限后分页返回
setup:
- name: login
...
assertions:
  ...
  - scenario: seatId 为空
    trigger: 请求缺参
    expect: $.status=="ERROR"（参数校验失败，Assert.notNull 抛 IllegalArgumentException）
  - scenario: 无权限
    trigger: 权限校验抛错
...
params:
  ...
  - name: desktop_name
    ...
    used_by: 见 setup/request
---
# POST /rcc/classroom/seat/disk/list

> 查看座位终端的磁盘信息列表，按座位ID校验权限后分页返回 ｜ 无特殊权限 ｜ 同步（分页查询）
...
graph LR
    ...
    B["POST /rcc/classroom/seat/disk/list<br>查看座位终端的磁盘信息列表，按座位ID校验权限后分页返回<br>权限: 无"]
    A1 -->|数据| B
    subgraph 内部处理流程
        C1["Step1: Assert.notNull 校验 request/sessionContext"]
        C2["Step2: rccPermissionChecker.checkTerminalGroupP"]
        C3["Step3: seatAPI.getTerminalDiskList(seatId) 查询磁盘"]
        C4["Step4: stream.map(TCITerminalDiskInfoDTO::conve"]
        C5["Step5: 构造 PageQueryResponse 返回"]
        C1 --> C2
        C2 --> C3
        C3 --> C4
        C4 --> C5
...
| 权限注解 | 无 |
| 执行方式 | 同步（分页查询） |
| 业务含义 | 查看座位终端的磁盘信息列表，按座位ID校验权限后分页返回 |
...
| page | Integer | 否 | @Range(min=0) | 页码（继承，默认0） |
| limit | Integer | 否 | @Range(min=1,max=2000) | 每页条数（继承，默认1） |
| matchArr | Match[] | 否 | @NotNull 默认空数组 | 匹配条件（继承） |
| sortArr | Sort[] | 否 | @NotNull 默认空数组 | 排序条件（继承） |
...
3. seatAPI.getTerminalDiskList(seatId) 查询磁盘列表
4. stream.map(TCITerminalDiskInfoDTO::convertFrom) 转换为 TCI 磁盘DTO
5. 构造 PageQueryResponse 返回
...
| 场景 | 触发条件 | 断言点 |
|---|---|---|
| seatId 为空 | 请求缺参 | $.status=="ERROR"（参数校验失败，Assert.notNull 抛 IllegalArgumentException） |
