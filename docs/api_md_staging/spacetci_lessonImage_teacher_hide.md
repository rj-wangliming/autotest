...
version: '2.0'
api:
  url: /spacetci/lessonImage/teacher/hide
  method: POST
  name: 隐藏教师机课程镜像：变更隐藏状态为true并同步推送镜像列表到教师机
  controller: TCILessonImageController
  method_ref: hideTeacherLessonImage
  permission: '@EnableAuthority'
  exec_mode: sync
  async: false
  description: 隐藏教师机课程镜像：变更隐藏状态为true并同步推送镜像列表到教师机
setup:
- name: login
...
response:
  ...
  body:
    msgKey:
      type: String
      description: spacetci_lessonimage_hide_teacher_image_success_log
upstream:
- api: POST /spacetci/lessonImage/getLessonImageList
...
assertions:
  success:
  - scenario: 隐藏教师机镜像
    expect: $.status==SUCCESS（content 为空，msgKey==spacetci_lessonimage_hide_teacher_image_success_log）
  failure:
  - scenario: 操作学生机镜像
    trigger: assertTeacherImageTrue抛62110029
    expect: $.status==ERROR && $.msgKey==spacetci_lessonimage_hide_teacher_image_fail_log
cleanup:
- api: POST /spacetci/lessonImage/teacher/show
  note: 误隐藏时通过 show 接口恢复（反向操作）
idempotency:
  level: data_level
  note: 重复隐藏状态幂等，但每次触发教师机推送
params:
  required:
...
---
# POST /spacetci/lessonImage/teacher/hide

> 隐藏教师机课程镜像：变更隐藏状态为true并同步推送镜像列表到教师机 ｜ @EnableAuthority ｜ sync
...
graph LR
    ...
    B["POST /spacetci/lessonImage/teacher/hide<br>隐藏教师机课程镜像：变更隐藏状态为true并同步推送镜像列表到教师机<br>权限: @EnableAuthority"]
    A1 -->|数据| B
    subgraph 内部处理流程
...
| URL | /spacetci/lessonImage/teacher/hide |
| Controller | TCILessonImageController |
| 方法名 | hideTeacherLessonImage |
| 权限注解 | @EnableAuthority |
| 执行方式 | sync |
| 业务含义 | 隐藏教师机课程镜像：变更隐藏状态为true并同步推送镜像列表到教师机 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| msgKey | String | spacetci_lessonimage_hide_teacher_image_success_log |
...
4. classroomAPI.changeLessonImageStatus(id,true) 隐藏
5. classroomTeacherAPI.pushClassroomImageToTeacher同步推送
6. 记录审计日志返回成功

## 下游消费方

### 消费1：POST /spacetci/lessonImage/teacher/hide
...
| 场景 | 断言点 |
|---|---|
| 隐藏教师机镜像 | $.status==SUCCESS（content 为空，msgKey==spacetci_lessonimage_hide_teacher_image_success_log） |

### 失败场景

| 场景 | 触发条件 | 断言点 |
|---|---|---|
| 操作学生机镜像 | assertTeacherImageTrue抛62110029 | $.status==ERROR && $.msgKey==spacetci_lessonimage_hide_teacher_image_fail_log |

## 环境清理机制

| 接口 | 说明 |
|---|---|
| POST /spacetci/lessonImage/teacher/show | 误隐藏时通过 show 接口恢复（反向操作） |
