...
version: '2.0'
api:
  url: /rcc/classroom/image/teacher/update
  method: POST
  name: 更新教师机课程镜像（action=UPDATE），同步执行镜像更新并通知教师机刷新镜像
  controller: RccClassroomImageController
  method_ref: teacherDoUpdate
  permission: '@EnableAuthority'
  exec_mode: sync
  async: false
  description: 更新教师机课程镜像（action=UPDATE），同步执行镜像更新并通知教师机刷新镜像
setup:
- name: login
...
assertions:
  success:
  - scenario: 镜像存在
    expect: $.status==SUCCESS（同步更新成功，content 为空，msgKey==rcdc_rcc_module_operate_success）
  failure:
  - scenario: 镜像不存在
    trigger: imageId 无效
...
params:
  ...
  - name: student_image_name
    ...
    used_by: 见 setup/request
---
# POST /rcc/classroom/image/teacher/update

> 更新教师机课程镜像（action=UPDATE），同步执行镜像更新并通知教师机刷新镜像 ｜ @EnableAuthority ｜ sync
...
graph LR
    ...
    B["POST /rcc/classroom/image/teacher/update<br>更新教师机课程镜像（action=UPDATE），同步执行镜像更新并通知教师机刷<br>权限: @EnableAuthority"]
    A1 -->|数据| B
    A2 -->|数据| B
    subgraph 内部处理流程
        ...
        C5["Step5: notifyImageInfo：教师机调用 classroomTeacherAP"]
...
| 执行方式 | sync |
| 业务含义 | 更新教师机课程镜像（action=UPDATE），同步执行镜像更新并通知教师机刷新镜像 |
...
| action | Integer | 是 | @NotNull，值1=UPDATE | 动作 |
| shouldOnlyDeleteDataFromDb | Boolean | 否 | 可空 | 仅删除场景使用 |
...
4. doImageAction()：构建 DoCardActionRequest → classroomImageAPI.doAction(request) 执行更新
5. notifyImageInfo：教师机调用 classroomTeacherAPI.pushClassroomImageToTeacher(classroomId)
...
| 场景 | 断言点 |
|---|---|
| 镜像存在 | $.status==SUCCESS（同步更新成功，content 为空，msgKey==rcdc_rcc_module_operate_success） |
